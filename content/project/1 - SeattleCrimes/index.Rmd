...
crimes = separate(crimes, Time, c("Hour", "Min"), sep = "[:]")
```


```{r, echo=FALSE}
# Remove 2021 data, only taking complete year reports and removed crimes occured before 2008, due to very small numbers (post-dated reporting)
...
kable(crimes.2020.burglary.top, caption = "Top 5 Address with the highest number of Burglary/Breaking.", col.names = c("Longitude", "Latitude", "Total Count", "Street Address"))
```
...
*One should keep in mind that the addresses generated are an approximation based on the Latitude and Longitude coordinates*. However, it gives a pretty clear picture of where burglary tend to occur. Our hypothesis seem to be holding, commercial properties are most likely the target for burglary and breaking crimes.


##### Prime Crime hours

Do different crimes have different occurence patterns throughout the day?
